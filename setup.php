...
out('|'.str_pad('  SIDEWA - SETUP APLIKASI', 63).'|', 'bold', 'green');
out('+'.str_repeat('-', 63).'+', 'bold', 'green');
out('');
info("Lokasi  : {$dir}");
info('PHP     : '.phpversion());
...
$db = [
    ...
    'DATABASE' => input('Nama Database', 'sidewa'),
...
out('');
out('+'.str_repeat('-', 63).'+', 'bold', 'green');
out('|'.str_pad(' SETUP APLIKASI SIDEWA BERHASIL DISELESAIKAN!', 63).'|', 'bold', 'green');
out('+'.str_repeat('-', 63).'+', 'bold', 'green');
out('');
